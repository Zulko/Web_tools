...
def check_barcode_skippedwells(filepath, xml_files, barcode, s_or_d):
    ...
    for xml in xml_files:
        try:
            ...
            if num_skippedwells > 0 and barcode == plate_barcode and dif_time.days < 20:
...
def main(argv):
    if len(sys.argv) > 2:
        ...
        filepath = os.path.join('T:\\Output', 'Echo')
        ...
        if len(xml_files) > 0:
            files_skippedwells = check_barcode_skippedwells(filepath, xml_files, barcode, source_or_destination)
