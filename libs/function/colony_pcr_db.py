...
def verify_samples_volume(vol_for_part, count_unique_list, found_list, robot):
    alert = []
...
def run_colony_pcr_db(path, filename, dispenser_parameters, mix_parameters, out_num_well, pattern, use_high_low_chip_mantis, user, scriptname):
    ...
    if len(missing_list) > 0:
        ...
        # list_source_wells, list_part_low_vol, alert = verify_samples_volume(vol_for_part, count_unique_list, found_list, robot)
        # if len(alert) > 0:
        #     for item in alert:
